...
function moverCenarioParaDireita() {
    ...
    let posicisaoFinalDireitaCenario = parseInt(imgCenario.style.width.replace("px","")) - centroTelaHorizontal - metadeLarguraAvatar
    if (Math.abs(leftCenario) < posicisaoFinalDireitaCenario) {
...
function moverCenarioParaBaixo() {
    let topCenario = parseInt(imgCenario.style.top.replace("px",""));
    let posicisaoFinalTopCenario = parseInt(imgCenario.style.height.replace("px","")) - centroTelaVertical - metadeLarguraAvatar
    if (Math.abs(topCenario) < posicisaoFinalTopCenario) {
        imgCenario.style.top = (topCenario - tamanhoMovimento) + "px";
    }
}
